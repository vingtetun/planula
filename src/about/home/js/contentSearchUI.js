...
ContentSearchUIController.prototype = {
  ...
  _onContentSearchService: function (event) {
    console.log('Call onContentSearchService: ', event.data.type, ': ', event.data.data);
    let methodName = "_onMsg" + event.data.type;
    if (methodName in this) {
      this[methodName](event.data.data);
    }
  },
  ...
  _sendMsg: function (type, data=null) {
    if (!this._channel) {
      this._channel = new BroadcastChannel('SearchService'),
      this._channel.onmessage = this._onContentSearchService.bind(this);
    }

    dump('Send Msg: ' + type + ': ' + data + '\n');
    console.log('Send Msg: ', type, ': ', data);
...
SearchService.onmessage = function(msg) {
  console.log('Content Receive message: ', msg.data.type, ': ', msg.data.data);
  switch (msg.data.type) {
    case 'GetState':
      SearchService.postMessage({
        type: msg.data.type.replace('Get', ''),
        data: {
          engines: [
            {
              name: 'Foo',
    ...
    case 'GetSuggestions':
      let rv = {
        engineName: 'Foo',
        searchString: msg.data.data.searchString,
        formHistory: [],
        remote: ['yo', 'yi', 'ya']
      };
      SearchService.postMessage({
